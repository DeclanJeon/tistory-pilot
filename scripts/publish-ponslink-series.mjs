...
function buildArgs(post, options) {
  ...
  args.push('--category', options.category || post.category || '개발 회고');
...
async function runPost(post, index, total, options) {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on('exit', code => {
      ...
      reject(new Error(`시리즈 발행 실패: ${post.title} (exit ${code})`));
    });
  });
}
...
async function main() {
  ...
  await ensureCategory({ ...options, category });

  for (let index = options.startAt; index < manifest.posts.length; index += 1) {
    await runPost(manifest.posts[index], index, manifest.posts.length, { ...options, category });
  }
}
