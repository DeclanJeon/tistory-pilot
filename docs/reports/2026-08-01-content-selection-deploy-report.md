...
중복으로 생성된 `/952`(에어컨)와 `/954`(이사 추가요금)는 관리자 화면에서 삭제했으며, 두 URL 모두 HTTP 404를 확인했다. 최신 `/957`, `/955`와 `/956`, `/953`만 유지된다.
 
---

## 11. 승인 후 일일 15건 운영 최종 확인 (2026-08-01 22:12 KST)

- 일일 큐 캡을 5건에서 15건으로 변경하고 서버에 배포
- 추가 생성 시도 15건 중 QA 통과 11건을 신규 큐에 등록
- 신규 Job 11건 생성 성공
- 카테고리 누락으로 실패한 3건은 `생활·정보` 카테고리 생성 후 재시도
- 최종 신규 Job 11건 `succeeded`
- 기존 고유 발행 4건 + 신규 11건 = 오늘 고유 발행 **15건**
- 관리자 글 목록에서 오늘 날짜 게시글 15건 확인
- 신규 공개 URL: `/958` ~ `/968`
- QA 실패 콘텐츠 4건은 큐에 등록하지 않음

최종 운영 정책: 애드센스 승인 사이트는 일일 최대 15건까지 허용하되, 각 글은 생성 QA·발행 직전 QA·YMYL·울타리·믹스 게이트를 모두 통과해야 한다.
